...
export function findManagedTomlBlocks(
  ...
  const isStart = (index: number): boolean =>
    lines[index].text.trim().startsWith(markers.startMarker)
  const isEnd = (index: number): boolean => lines[index].text.trim().startsWith(markers.endMarker)
