...
describe('cron bare stepped values (#15723)', () => {
  ...
  // neither the oversized-step expansion (#15895) nor the day-restriction flags (#15896).
...
describe('cron oversized steps (#15895)', () => {
  ...
  // An oversized field step is not an elapsed interval: it is one value, not "every 90 minutes".
  it('degenerates a persisted oversized step to its single value rather than throwing', () => {
    expect(ascending(parseCronExpression('*/90 * * * *').minutes)).toEqual([0])
    expect(
...
describe('cron occurrence local-time controls', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
  })
  ...
  it('fires both repeats of the wall-clock hour local fall-back replays', () => {
    ...
    // Pre-existing limit this change does not touch or fix: local 01:30 is ambiguous, so
    // flooring it rebuilds the earlier EDT instant and the scan lands back on the EST repeat
